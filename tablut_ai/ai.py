...
def update_player_pieces(board):
    ...
    player_pieces[2] = [
        i for i in range(len(board)) if board[i] == 2 or board[i] == 3
    ]  # Include king for player 2
...
def get_possible_moves(board, player):
    ...
    for i in range(len(board)):
        if board[i] == player or (
            player == 2 and board[i] == 3
        ):  # Include king for player 2
...
def get_best_move(board, player, depth=3):
    ...
    for move in get_possible_moves(board, player):
        ...
        if (player == 2 and move_value > best_value) or (
